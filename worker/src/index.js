...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname !== '/api/stripe-webhook') {
      ...
      const isAccountsRead = ['/api/accounts/list','/api/accounts/summary','/api/accounts/journal','/api/accounts/statements','/api/accounts/reconciliation/summary','/api/accounts/reconciliation/list','/api/accounts/invoices','/api/accounts/invoices/detail','/api/accounts/quotes','/api/accounts/quotes/detail','/api/accounts/recurring','/api/accounts/rules','/api/accounts/close-checklist','/api/accounts/lock-periods'].includes(url.pathname) && request.method === 'GET';
      const isAccountsWrite = ['/api/accounts/journal','/api/accounts/rebuild-auto-journal','/api/accounts/year-close','/api/accounts/reconciliation/import','/api/accounts/reconciliation/match','/api/accounts/invoices','/api/accounts/invoices/update','/api/accounts/invoices/status','/api/accounts/invoices/payment','/api/accounts/invoices/send','/api/accounts/invoices/delete','/api/accounts/quotes','/api/accounts/quotes/update','/api/accounts/quotes/delete','/api/accounts/quotes/send','/api/accounts/quotes/convert','/api/accounts/recurring','/api/accounts/recurring/apply','/api/accounts/rules','/api/accounts/close-checklist','/api/accounts/lock-periods'].includes(url.pathname) && request.method === 'POST';
    ...
    if (url.pathname === '/api/accounts/invoices/update' && request.method === 'POST') {
      return handleInvoiceUpdate(request, env, corsHeaders);
    }
    ...
    if (url.pathname === '/api/accounts/invoices/send' && request.method === 'POST') {
      return handleInvoiceSend(request, env, corsHeaders);
    }
...
async function handleTaxExportCsv(request, env, corsHeaders, url) {
  ...
  const filename = `admin-sandbox-tax-${year}-${type}.csv`;
